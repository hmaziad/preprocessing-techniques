...
def getHistoryPercentageWithKeepOnlyDuplicates(dialogues, percentage):
    ...
    for dialogue in dialogues:
        ...
        for pair in dialogue:
            ...
            intersectionOfWords = getCommonTermsIntersection(request.strip())
            if(len(intersectionOfWords) == 0):
                intersectionOfWords = restoWordsPrevPair
            ...
            pair[0] = (" ".join(nonRestoWords).strip() + " " + " ".join(intersectionOfWords).strip()).strip()
            # print("Result: ", pair[0] )
            pairAsArr =  pair[0].split(" ")
            prevUserRequest = pairAsArr[: int(len(pairAsArr) // (float(100 / percentage)))]
    return dialogues
...
# for historyPercentage in [100]: # history percentage
...
# newCustomPath  =   ROOT_DIR + "/data/history_converge/100_history/k_5/iteration_0/alldialogues_trn.p"
printRandomSample(getHistoryPercentageWithKeepOnlyDuplicates(parseDialogueFromFile(srcDialoguesPath),100))
